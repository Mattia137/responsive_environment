/* =========================================================================
   ALLSPARK // IMPACT  ·  layers/dispatch.js
   Converts impact-model spatial output to MapLibre sources + layers.
   --------------------------------------------------------------------------
   Called from main.js on every refresh. Paint properties read CSS custom
   properties so the layers flip on theme change.
   ========================================================================= */
...
export function renderActiveLayers(state) {
  // 1. First, clear any layer sources whose layer is not currently active
  LAYERS.forEach(L => {
...
function addFlowLayer(layerId, srcId) {
  ...
  // arrowheads as circles at line endpoints — implement in layer-specific code
}
